...
def compute_regression_metrics_log(predictions, targets):
    try:
        predictions = np.where(predictions > 0, predictions, 0.01)
        predictions_log = -np.log10(predictions)
        targets_log = -np.log10(targets)

        mse = mean_squared_error(targets_log, predictions_log)
        mae = mean_absolute_error(targets_log, predictions_log)
        rmse = np.sqrt(mse)
        corr = r2_score(targets_log, predictions_log)
...
class TukeysBiweightLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        loss = torch.where(torch.abs(residual) < self.c, 
                           (self.c**2 / 6) * (1 - (1 - (residual / self.c)**2)**3),
                           (self.c**2 / 6))
        return loss.mean()
...
def main():
    ...
    taskname='1022_8_3mod_fusion_maxavg_folder'
    print(taskname)
    #3_allavg    3_avglinear  3_allmaxavg
    modeltype="3_allavg"

    #Huber Loss   nn.MSELoss
# Huber Loss（平滑的L1损失）：Huber Loss结合了均方误差和均值绝对误差的特性，对异常值更具鲁棒性。它在PyTorch中使用Huber Loss  nn.SmoothL1Loss实现，如之前所示。

# Tukey's Biweight Loss：这是一种可调整鲁棒性的损失函数，你可以通过调整参数c来控制其对异常值的敏感度，更大的c值使其对异常值更具鲁棒性。

# Quantile Loss（分位数损失）：在分位数回归中，你可以选择适当的分位数来捕捉异常值。较高分位数（例如第90百分位数）的损失函数通常对异常值更敏感。

# Log-Cosh Loss：这是一个平滑的损失函数，对于异常值相对均方误差更具鲁棒性，因为它是基于对数的。

#LogCoshLoss    HuberLoss  LogCoshLoss  QuantileLoss  MSELoss
    losstype="MSELoss"
    ...
    net = Build_MultiModel_ShareBackbone_3mod(backbone=modeltype)
    ...
    elif losstype=="HuberLoss":
        loss_function = nn.SmoothL1Loss()
    elif losstype=="TukeysBiweightLoss":
        loss_function = TukeysBiweightLoss(c=8)
    elif losstype=="LogCoshLoss":
        loss_function = LogCoshLoss()
    elif losstype=="QuantileLoss":
    # 选择分位数水平，例如 0.9 表示第 90 百分位数
        quantile = 0.3
        loss_function = QuantileLoss(quantile)
    ...
    for epoch in range(epochs):
        ...
        for count, (hbimages, spimages,ydimg,target,_) in enumerate(train_bar):

            target = target.to(device).float()
            target = target.view(-1, 1)
            
            logits = net(hbimages, spimages,ydimg)
        ...
        with torch.no_grad():
            ...
            for val_data in val_bar:
                ...
                #20张图片最初作为对象扔进来，方法：
                outputs = net(hbimages, spimages,ydimg)
