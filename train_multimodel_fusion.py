...
def main():
    ...
    for epoch in range(epochs):
        ...
        for count, (hbimages, spimages,target,_) in enumerate(train_bar):

            target = target.to(device).float()
            target = target.view(-1, 1)
            
            logits = net(hbimages, spimages)
