...
from fusion_multi_model import Build_MultiModel_ShareBackbone_octyd
...
class CustomDataset_Multi(torch.utils.data.Dataset):
    def __init__(self, im_dir, hb_im_names, yd_im_names,im_labels, im_path,im_transforms=None):
        self.im_dir = im_dir
    ...
    def __getitem__(self, idx):
        ...
        yd_image=self.yd_im_names[idx].split(';')[0]

        hbmodified_list = [os.path.join(self.im_dir,self.im_path_head[idx],'OCT', string) for string in hb_image_list]
            
        # yd_image_path = os.path.join(self.im_dir,self.im_path_head[idx],'眼底照相', yd_image) 
        ...
        return hbimages,ydimage,self.im_labels[idx], self.im_path_head[idx]
...
def load_data_multi(label_path, train_lists, img_path,
    ...
    for train_list in train_lists:
        ...
        train_sets.append(CustomDataset_Multi(img_path, hb_im_names, yd_im_names,im_labels , im_path,im_transforms))
...
def main():
    ...
    taskname='1113_hbyd_fusion_avglin'
    print(taskname)
    #resnest50   FITNet  resnest50cbma  resnest50cbma2  resnest50next  resnest50stage  hbsp_avglinear  octyd_avgavg   octyd_avglin
    modeltype="octyd_avglin"

    print(modeltype)



    label_path='/root/work2023/deep-learning-for-image-processing-master/data_set/TRSL_ALL/1113'
    ...
    net = Build_MultiModel_ShareBackbone_octyd(backbone=modeltype)


    # if modeltype=="resnest50_3d":
    #     for param in net.model_oct.parameters():
    #         param.requires_grad = False
    #     # for param in net.model_yd.parameters():
    #     #     param.requires_grad = False
    ...
    for epoch in range(epochs):
        ...
        for count, (hbimages, ydimg,target,_) in enumerate(train_bar):

            target = target.to(device).float()
            target = target.view(-1, 1)
            
            logits = net(hbimages, ydimg)
        ...
        with torch.no_grad():
            ...
            for val_data in val_bar:
                hbimages, ydimg,val_labels,imgids = val_data
